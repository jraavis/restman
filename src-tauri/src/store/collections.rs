...
const SELECT: &str =
    "SELECT id, workspace_id, parent_id, name, description, sort_order, created_at, updated_at FROM collections";
...
fn copy_children(conn: &Connection, from_id: &str, to_id: &str) -> AppResult<()> {
    let mut req_stmt = conn.prepare(
        "SELECT name, method, url, headers_json, query_json, body_json, options_json FROM requests WHERE collection_id = ?1 ORDER BY sort_order ASC",
    )?;
    let requests: Vec<(String, String, String, String, String, String, String)> = req_stmt
        .query_map(params![from_id], |r| {
            Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?, r.get(5)?, r.get(6)?))
        })?
        .collect::<rusqlite::Result<Vec<_>>>()?;
    let now = now_millis();
    for (i, (name, method, url, headers_json, query_json, body_json, options_json)) in requests.into_iter().enumerate() {
        conn.execute(
            ...
            params![Uuid::new_v4().to_string(), to_id, name, method, url, headers_json, query_json, body_json, options_json, i as i64, now],
        )?;
    }

    let mut child_stmt = conn.prepare("SELECT id, name, description FROM collections WHERE parent_id = ?1 ORDER BY sort_order ASC")?;
    let children: Vec<(String, String, Option<String>)> = child_stmt
        .query_map(params![from_id], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)))?
        .collect::<rusqlite::Result<Vec<_>>>()?;
    for (child_id, child_name, child_desc) in children {
